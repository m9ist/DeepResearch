...
(кнопка «+N»), `worker_model` (применяется к новым Worker'ам). Остальное —
при старте / между раундами. `round_time_budget_sec.first` короче — быстрый
первый результат (5–10 мин).
...
2. Запуск Run'а из веб-морды нарезает Backlog и гонит Round 1 в пределах
   `round_time_budget_sec.first`, спавня ≤ `concurrency` Worker'ов.
